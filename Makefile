...
.PHONY: all clean clean-all clean-test clean-dist clean-cache \
        ...
        all-extra yt-dlp-extra current-ejs-version current-protobug-version
...
clean-dist:
	...
	yt-dlp.zip .ejs-* .protobug-* yt_dlp_ejs/ protobug/
clean-cache:
	find . \( \
...
# The following PROTOBUG_-prefixed variables are auto-generated by devscripts/update_protobug.py
# DO NOT EDIT!
PROTOBUG_VERSION = 1.0.0
PROTOBUG_WHEEL_NAME = protobug-1.0.0-py3-none-any.whl
PROTOBUG_WHEEL_HASH = sha256:8b1b5cb3f4b2dd4fa0a6219ca770f687d0af32606d344ac7acedf00943f220c3
PROTOBUG_PY_FOLDERS = protobug
PROTOBUG_PY_FILES = protobug/__init__.py protobug/_core.py protobug/_reader.py protobug/_version.py protobug/_writer.py

.protobug-$(PROTOBUG_VERSION):
	@echo Downloading protobug
	@echo "protobug==$(PROTOBUG_VERSION) --hash $(PROTOBUG_WHEEL_HASH)" > .protobug-requirements.txt
	$(PYTHON) -m pip download -d ./build --no-deps --require-hashes -r .protobug-requirements.txt
	unzip -o build/$(PROTOBUG_WHEEL_NAME) "protobug/*"
	@touch .protobug-$(PROTOBUG_VERSION)

current-protobug-version:
	rm -rf .protobug-*
	touch .protobug-$$($(PYTHON) -c 'import sys; sys.path = [""]; from protobug import __version__; print(__version__)' 2>/dev/null)

yt-dlp-extra: current-ejs-version .ejs-$(EJS_VERSION) $(EJS_PY_FILES) $(EJS_JS_FILES) current-protobug-version .protobug-$(PROTOBUG_VERSION) $(PROTOBUG_PY_FILES) yt-dlp.zip
	mkdir -p zip
	...
	for d in $(PROTOBUG_PY_FOLDERS) ; do \
	  mkdir -p zip/$$d ;\
	  cp -pPR $$d/*.py zip/$$d/ ;\
	done
	(cd zip && touch -t 200001010101 $(EJS_PY_FILES) $(EJS_JS_FILES) $(PROTOBUG_PY_FILES))
	(cd zip && zip -q ../yt-dlp.zip $(EJS_PY_FILES) $(EJS_JS_FILES) $(PROTOBUG_PY_FILES))
	cp -pP yt_dlp/__main__.py zip/
	touch -t 200001010101 zip/__main__.py
	(cd zip && zip -q ../yt-dlp.zip __main__.py)
	echo '#!$(PYTHON)' > yt-dlp
