...
class TestMediaHeader:
    ...
    def test_previous_ooo_initialized_format_discard(self, base_args):
        ...
        # Clear consumed ranges. We want to also handle the case when we cannot mark the format as entirely consumed.
        initialized_format.consumed_ranges.clear()
    ...
    def test_multiple_consumed_ranges(self, base_args):
        ...
        # Consumed segments in the same chain BEFORE the previous segment should be accepted (and ignored)
        for seq_no in [1, 2, 3]:
            result = processor.process_media_header(make_media_header(selector, sequence_no=seq_no))
            assert isinstance(result, ProcessMediaHeaderResult)
            assert result.sabr_part is None
            processor.partial_segments.clear()

    def test_multiple_consumed_ranges_previous_segment_consumed(self, base_args):
        # Allow next segment to be at the end of a chain of consumed ranges even if previous segment is consumed
        # (similar to above)
        selector = make_selector('audio')
        processor = SabrProcessor(
            **base_args,
        ...
        # Previous segment was segment 4 and consumed
        initialized_format.previous_segment = Segment(
            format_id=selector.format_ids[0],
            is_init_segment=False,
            sequence_number=4,
        ...
        # Segment 5 should be ignored as already consumed
    ...
    def test_error_on_mismatch_consumed_segment_before_previous(self, base_args):
        # Should raise mismatch error if:
        # 1. Previous segment is consumed
        # 2. New segment is also consumed
        # 3. New segment is before previous segment (out of order) and not in the same consumed range chain
        ...
        fim = make_format_im(selector)
        processor.process_format_initialization_metadata(fim)
        # Simulate that the previous segment was consumed
        initialized_format = processor.initialized_formats[str(selector.format_ids[0])]
        initialized_format.previous_segment = Segment(
            format_id=selector.format_ids[0],
            is_init_segment=False,
        ...
        # Mark segment 3 and 5 as consumed
        initialized_format.consumed_ranges.append(
            ConsumedRange(
                start_sequence_number=3,
    ...
    def test_error_on_mismatch_consumed_segment_after_previous(self, base_args):
        # Should raise mismatch error if:
        # 1. Previous segment is consumed
        # 2. New segment is also consumed
        # 3. New segment is after previous segment (out of order) and not in the same consumed range chain
        selector = make_selector('audio')
        processor = SabrProcessor(
            **base_args,
            audio_selection=selector,
        ...
        # Mark segment 7 and 5 as consumed
        initialized_format.consumed_ranges.append(
            ConsumedRange(
                start_sequence_number=7,
    ...
    def test_error_mismatch_not_start_at_expected(self, base_args):
        # Should raise mismatch error if:
    ...
    def test_no_error_mismatch_not_start_at_expected_consumed(self, base_args):
        # Same as above, but the first segment is already consumed
    ...
    def test_no_error_mismatch_not_start_at_one_live(self, base_args):
        ...
        # 3. live
        # 4. New segment sequence number is not 1
        # Note: usually if live and segments are not starting at 0,
        # player_time_ms would have been seeked to a later time
        selector = make_selector('audio')
        processor = SabrProcessor(
            **base_args,
            audio_selection=selector,
    ...
    def test_no_error_mismatch_not_start_at_one_postlive(self, base_args):
        ...
        fim = make_format_im(selector)
        processor.process_format_initialization_metadata(fim)
        # Simulate player time progressed
        assert processor.client_abr_state.player_time_ms == 0
        # Should not raise an error
        result = processor.process_media_header(make_media_header(selector, sequence_no=2))
        assert isinstance(result, ProcessMediaHeaderResult)
...
class TestMedia:
    ...
    def test_consumed_partial_segment(self, base_args):
        # Should ignore the media part if the segment is marked as consumed
        selector = make_selector('audio')
        processor = SabrProcessor(
            **base_args,
            audio_selection=selector,
        ...
        processor.partial_segments[media_header.header_id].consumed = True
